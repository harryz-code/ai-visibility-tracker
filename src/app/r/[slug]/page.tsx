...
function resolveReport(slug: string): ReportPayload {
  ...
  // Rebuild from slug pattern: brand-category-timestamp
  const parts = slug.split("-");
  if (parts.length >= 2) {
    const brand = parts[0] ? capitalize(parts[0]) : "Affirm";
    const category = parts[1] ? parts[1].toUpperCase() === "bnpl" ? "BNPL" : capitalize(parts[1]) : "BNPL";
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-10 px-6 py-10">
      ...
      <section className="rounded-lg border border-zinc-200 bg-white p-6">
        <h2 className="text-lg font-medium text-zinc-900">Per-model breakdown</h2>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-lg border border-zinc-200 bg-white p-6">
          ...
          <p className="mb-4 text-sm text-zinc-500">
            Auto-detected from co-mentions in the same answers.
          </p>
